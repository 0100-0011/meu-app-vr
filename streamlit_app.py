...
def consolidar_bases(dataframes):
    ...
    if 'DIAS_UTEIS' in dataframes:
        ...
        if 'SINDICADO' in dias_uteis_df.columns:
            dias_uteis_df = dias_uteis_df.rename(columns={'SINDICADO':'Sindicato'})
    ...
    exclusao_matriculas = []
    def coletar_matriculas(df, col='MATRICULA'):
        if df is not None and col in df.columns:
            exclusao_matriculas.extend(df[col].dropna().astype(str).tolist())

    for t in ['ESTAGIARIOS','APRENDIZES','AFASTAMENTOS','EXTERIOR']:
        if t in dataframes:
            df_t = dataframes[t]
            if 'MATRICULA' in df_t.columns:
                coletar_matriculas(df_t,'MATRICULA')
            elif 'Matrícula' in df_t.columns:
                coletar_matriculas(df_t,'Matrícula')
...
def gerar_vr_com_langchain(consolidated_df):
    ...
    # Parsing da resposta não implementado, retornando df original
    return consolidated_df
    ...
    if uploaded_files:
        with st.spinner("Carregando e consolidando bases..."):
            ...
            if consolidated_df is None or consolidated_df.empty:
                st.error("Erro ao consolidar dados. Verifique os arquivos.")
            else:
